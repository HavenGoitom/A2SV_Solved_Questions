class NumMatrix:
    ...
    def __init__(self, matrix: List[List[int]]):
        ...
        self.prefix_sum = [[0]*(len(self.matrix[0])+1) for i in range(len(self.matrix)+1)]

        for i in range(1, len(self.prefix_sum)):
            for j in range(1,len(self.prefix_sum[0])):
                self.prefix_sum[i][j] = self.matrix[i-1][j-1] + (self.prefix_sum[i][j-1] + self.prefix_sum[i-1][j]) - self.prefix_sum[i-1][j-1]
